...
export function createSound({ reduced = false } = {}) {
  ...
  function build() {
    ...
    sparks = SPARK[scene].map((m, k) => {
      ...
      const g = ctx.createGain(); g.gain.value = 0.0028; lfo(0.05 + k * 0.03, 0.0028, g.gain);
    ...
    airGain = ctx.createGain(); airGain.gain.value = 0.003;
  ...
  return {
    ...
    speed(v) {
      ...
      airGain.gain.setTargetAtTime(0.003 + k * 0.022, now, 0.35);
