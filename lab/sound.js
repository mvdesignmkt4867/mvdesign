/* ============================================================
   MV Design · Lab — sonido celestial + "Om" generado en vivo (Web Audio)
   ...
   · Monjes: canto "Om" en re grave debajo del coro; cada paso del scroll es un nuevo "Om" (A → U → M)
     y quieto se sostiene en "Mmm".
...
const OM = {
  a: { f: [700, 1150, 2600], g: [3.0, 2.0, 0.7], n: 1.0 },   // (el grave "n" casi fijo: la vocal colorea, no hace bajones)
  u: { f: [330, 820, 2250], g: [2.8, 1.0, 0.18], n: 1.05 },
  m: { f: [260, 700, 2200], g: [0.5, 0.08, 0.001], n: 1.15 }
};
const KEY = "mv-sound";
const VOL = 0.74;
...
export function createSound({ reduced = false } = {}) {
  ...
  function build() {
    ...
    [[38, -6, 0.06], [38, 6, 0.06], [45, 3, 0.032], [26, 0, 0.02]].forEach(([m, c, g0]) => {
  ...
  function startMusic() {
    ...
    om(now + 0.4);                                           // el primer "Om" de los monjes
  }
  ...
  function om(t) {
    ...
    chantAmp.gain.setTargetAtTime(0.85, t, 0.08);   // (respiración sutil)
  ...
  function goMel(i, now) {
    ...
    om(now);                                                 // y los monjes entonan un nuevo "Om"
    lastStepT = now;
  }
